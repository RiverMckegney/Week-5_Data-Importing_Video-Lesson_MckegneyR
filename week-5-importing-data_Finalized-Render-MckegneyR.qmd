...
# readr is already contained as a pre-loaded core-package within the tidyverse package
```
...
```{r}
#| label: read-nimbus
nimbus <- read_csv("Raw_Data/nimbus.csv")
```
...
nimbus <- read_csv("Raw_Data/nimbus.csv", na = ".")
```
...
```{r}
write_csv(nimbus, file = "Worked_Data/nimbus_worked.csv")

worked_nimbus <- read_csv("Worked_Data/nimbus_worked.csv")
```
...
```{r}
#| label: load-gsw
gsw <- read_csv("Raw_Data/GSWdata.csv",
                na = "N/A",
                col_types = list('2020-21' = col_number(),
                                 'FG%' = col_number(),
                                 '3P%' = col_number(),
                              #  '2019-20' = col_number(), 
                                 '2019-20' = col_skip()))

# Add in the fixes above:
...
write_csv(gsw, file = "Worked_Data/gsw_worked.csv")
```
...
# Plot displaying missing data
ggplot(data = airquality,
       mapping = aes(x = Ozone,
                     y = Solar.R)) +
 geom_miss_point()

# Plot w/ missing data faceted by Month (suss out where NAs are occurring)
ggplot(data = airquality,
       mapping = aes(x = Ozone,
                     y = Solar.R)) +
 geom_miss_point() +
...
## Package Citation
1. Müller K, Wickham H (2024). tibble: Simple Data Frames. R package version 3.2.1, https://github.com/tidyverse/tibble, <https://tibble.tidyverse.org/>.
2. Wickham H, Averick M, Bryan J, Chang W, McGowan LD, François R, Grolemund
  ...
  Wilke C, Woo K, Yutani H (2019). “Welcome to the tidyverse.” _Journal of
  Open Source Software_, *4*(43), 1686. doi:10.21105/joss.01686
  <https://doi.org/10.21105/joss.01686>.
3.  Brownrigg DMPfRbR, Minka TP, Bivand. ttP9cbR (2023). _mapproj: Map
  Projections_. R package version 1.2.11,
  <https://CRAN.R-project.org/package=mapproj>.
4.  Becker OScbRA, Minka ARWRvbRBEbTP, Deckmyn. A (2024). _maps: Draw
  Geographical Maps_. R package version 3.4.2.1,
  <https://CRAN.R-project.org/package=maps>.
5. Tierney N, Cook D (2023). “Expanding Tidy Data Principles to Facilitate
  Missing Data Exploration, Visualization and Assessment of Imputations.”
  _Journal of Statistical Software_, *105*(7), 1-31. doi:10.18637/jss.v105.i07
  <https://doi.org/10.18637/jss.v105.i07>.
6. Firke S (2024). _janitor: Simple Tools for Examining and Cleaning Dirty
  Data_. R package version 2.2.1,
